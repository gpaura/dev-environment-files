...
local function split_current_csv(num_lines, command_name)
  ...
  vim.bo.filetype = current_ext == "csv" and "csv" or current_ext == "dat" and "csv" or current_ext == "tsv" and "csv" or "csv"
  
  local file_size = vim.fn.getfsize(new_filepath)
  vim.notify(string.format("✅ Created %s with %d lines (%s)", new_filename, #lines_to_save, format_bytes(file_size)), vim.log.levels.INFO)
...
vim.api.nvim_create_user_command("Details", function()
  ...
  local function format_delimiter(delim)
    if delim == "\t" then return "Tab"
    elseif delim == "," then return "Comma"
    elseif delim == ";" then return "Semicolon"
    elseif delim == "|" then return "Pipe"
    else return "'" .. delim .. "'" end
  end
